...
#endif
static irqreturn_t (*snull_interrupt)(int, void *);
...
int snull_open(struct net_device *dev)
{
	 ...
	 */
	memcpy(addr, "\0SNUL0", ETH_ALEN);
	if (dev == snull_devs[1])
		addr[ETH_ALEN-1]++; /* \0SNUL1 */
...
void snull_rx(struct net_device *dev, struct snull_packet *pkt)
{
	...
	if (!skb) {
		pr_notice_ratelimited("snull rx: low on mem - packet dropped\n");
...
static int snull_poll(struct napi_struct *napi, int budget)
{
	...
	while (npackets < budget && priv->rx_queue) {
		...
		skb = netdev_alloc_skb(dev, pkt->datalen + 2);
		if (! skb) {
			pr_notice_ratelimited("snull: packet dropped\n");
			priv->stats.rx_dropped++;
 ...
 */
void snull_get_stats64(struct net_device *dev, struct rtnl_link_stats64 *stats)
{
	struct snull_priv *priv = netdev_priv(dev);
...
#  define netif_napi_add_compat(dev, napi, poll) \
	netif_napi_add_weight(dev, napi, poll, 2)
#else
#  define netif_napi_add_compat(dev, napi, poll) \
	netif_napi_add(dev, napi, poll, 2)
#endif
...
void snull_init(struct net_device *dev)
{
	...
	priv->dev = dev;
	if (use_napi)
		netif_napi_add_compat(dev, &priv->napi, snull_poll);
...
void snull_cleanup(void)
{
	...
	for (i = 0; i < 2;  i++) {
		if (snull_devs[i]) {
			struct snull_priv *priv = netdev_priv(snull_devs[i]);
			if (use_napi)
